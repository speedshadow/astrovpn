...
export async function trackPageView() {
  ...
  try {
    ...
    if (!session) {
      console.error('[Analytics] Could not get or create a session. Aborting page view track.');
      return;
    }
...
export async function trackAffiliateClick(vpnId: number) {
  ...
  try {
    const session = await getOrCreateSession();
    if (!session) {
      console.error('[Analytics] Could not get or create a session. Aborting affiliate click track.');
      return;
    }
    ...
    console.error('[Analytics] A critical error occurred in trackAffiliateClick:', e);
  }
}
