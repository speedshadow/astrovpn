...
export async function fetchReviewPageBySlug(slug: string): Promise<PopulatedReviewPage | null> {
  ...
  if (page.vpn_ids && page.vpn_ids.length > 0) {
    const { data: vpnData, error: vpnError } = await supabase
      .from('vpns')
      .select('id,name,slug,logo_url,description,categories,star_rating,affiliate_link,price_monthly_usd,price_yearly_usd,price_monthly_eur,price_yearly_eur,detailed_ratings,supported_devices,pros,cons,keeps_logs,has_court_proof,has_double_vpn,has_coupon,show_on_homepage,court_proof_content,based_in_country_name,based_in_country_flag,coupon_code,coupon_validity,full_review,has_p2p,has_kill_switch,has_ad_blocker,has_split_tunneling,simultaneous_connections,server_count,country_count')
      .in('id', page.vpn_ids);
