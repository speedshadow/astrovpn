...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (!file.name.endsWith('.dump')) {
        return new Response(JSON.stringify({ error: 'Invalid file type. Only .dump files are allowed.' }), { status: 400 });
    }

    if (file.size > MAX_FILE_SIZE) {
      return new Response(JSON.stringify({ error: `File size exceeds the limit of ${MAX_FILE_SIZE / 1024 / 1024} MB` }), { status: 400 });
    }
  ...
  const restoreCommand = `cat "${tempFilePath}" | docker exec -i -e PGPASSWORD=${PGPASSWORD} ${CONTAINER_NAME} pg_restore --verbose --data-only --schema=public --disable-triggers -U supabase_admin -d postgres`;
    ...
    return new Response(JSON.stringify({ error: 'Restore failed', details: errorDetails }), { status: 500 });
