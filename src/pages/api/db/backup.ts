...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to create backup directory' }), { status: 500 });
  }
  ...
  try {
    ...
    const { stdout, stderr } = await execPromise(backupCommand, { encoding: 'buffer' });
    ...
    return new Response(JSON.stringify({ message: 'Backup created successfully', fileName: backupFileName }), { status: 200 });

  } catch (error: any) {
    console.error('Backup failed:', error);
    return new Response(JSON.stringify({ error: 'Backup creation failed', details: error.message }), { status: 500 });
  }
};
