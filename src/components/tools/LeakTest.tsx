...
import { CheckCircle2, AlertTriangle, XCircle, Loader2, MapPin, Server, Globe, RefreshCw } from 'lucide-react';
...
const LeakTest = () => {
    const [ipInfo, setIpInfo] = useState<IpInfo>({ ipv4: null, ipv6: null, city: null, country_name: null, org: null });
    ...
    return (
        <div className="space-y-8">
            <ResultCard title="Public IP Address" status={ipStatus}>
                ...
                {ipStatus === 'error' && (
                    <div className="text-center">
                         <p className="text-red-500 mb-4">Could not fetch your IP information.</p>
                ...
                {ipStatus === 'safe' && (
                    <div className="divide-y divide-gray-200 dark:divide-gray-700">
                        {ipInfo.ipv4 && <InfoRow icon={<Globe className="h-5 w-5" />} label="IPv4 Address" value={ipInfo.ipv4} />}
                        {ipInfo.ipv6 && <InfoRow icon={<Globe className="h-5 w-5" />} label="IPv6 Address" value={ipInfo.ipv6} />}
                        {ipInfo.city && ipInfo.country_name && <InfoRow icon={<MapPin className="h-5 w-5" />} label="Location" value={`${ipInfo.city}, ${ipInfo.country_name}`} />}
                        {ipInfo.org && <InfoRow icon={<Server className="h-5 w-5" />} label="Provider" value={ipInfo.org} />}
                    </div>
                )}
            </ResultCard>
            ...
            <ResultCard title="DNS Leak Test" status={dnsStatus}>
                ...
                {dnsStatus === 'error' && (
                    <div className="text-center">
                        <p className="text-red-500 mb-2">An error occurred while testing for DNS leaks.</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">This might be a network issue or a browser extension. Please check your connection and try again.</p>
                        <Button onClick={runIpAndDnsTests} variant="destructive" size="sm">
                            <RefreshCw className="h-4 w-4 mr-2" />
                            Try Again
                        </Button>
                ...
                {dnsStatus === 'results' && (
                    <div>
                        <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">Your DNS requests are handled by the servers below. If you're using a VPN, you should only see your VPN's DNS servers. If you see your regular ISP, you may have a DNS leak.</p>
                        {dnsServers.length > 0 ? (
                            <ul className="space-y-2">
                                {dnsServers.map((server, index) => (
                                    <li key={index} className="flex items-center gap-3 p-2 bg-gray-100 dark:bg-gray-900 rounded-md">
                                        <Server className="h-5 w-5 text-blue-500" />
                                        <div>
                                            <span className="font-mono text-sm">{server.ip}</span>
                                            <span className="text-xs text-gray-500 dark:text-gray-400"> ({server.country_name} - {server.isp})</span>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                           <p>No DNS servers detected. This could mean your connection is very secure, or there was an issue with the test.</p>
                        )}
                    </div>
                )}
            </ResultCard>
            ...
            <ResultCard title="WebRTC Leak Test" status={webrtcStatus}>
                ...
                {webrtcStatus === 'error' && <p className="text-red-500">WebRTC is not supported or an error occurred.</p>}
                {webrtcStatus === 'safe' && <p>No WebRTC leaks detected. Your real IP address is hidden.</p>}
                {webrtcStatus === 'leak' && (
                     <div>
                        <p className="mb-4">WebRTC leak detected! Your real IP address may be exposed.</p>
                        <ul className="space-y-2">
