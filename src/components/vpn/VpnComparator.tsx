...
async function fetchHomepageVpns(): Promise<Vpn[]> {
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch VPNs for comparator');
  }
  return response.json();
}

export const VpnComparator: React.FC = () => {
  const { data: vpns, isLoading, error } = useQuery<Vpn[]>({
    queryKey: ['homepageVpns'], // Use the same key to share cache
  ...
  const selectedVpns = useMemo(() => {
    if (!vpns) return [];
    return selectedIds.map(id => vpns.find((vpn: Vpn) => vpn.id === id)).filter(Boolean) as Vpn[];
  }, [selectedIds, vpns]);
  ...
  if (error) {
    return (
      <div className="w-full max-w-6xl mx-auto my-12 text-center text-red-500">
        <p>Failed to load VPN data for comparison.</p>
      </div>
    );
  }

  return (
    <Card id="vpn-comparator" className="w-full max-w-6xl mx-auto my-12">
      <CardHeader className="text-center">
        <CardTitle className="text-3xl md:text-4xl font-extrabold">Compare VPNs Side-by-Side</CardTitle>
        <p className="text-muted-foreground">Select up to 3 VPNs to compare features.</p>
      </CardHeader>
      <CardContent>
          ...
          {vpns?.map((vpn: Vpn) => (
            <div key={vpn.id} className="flex items-center space-x-2">
              <Checkbox
                ...
                disabled={!selectedIds.includes(vpn.id) && selectedIds.length >= 3}
              />
              <label
                htmlFor={`vpn-${vpn.id}`}
        ...
        {selectedVpns.length > 0 ? (
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="font-bold text-lg min-w-[150px]">Feature</TableHead>
                  {selectedVpns.map((vpn: Vpn) => (
                    <TableHead key={vpn.id} className="text-center min-w-[150px]">
                      <div className="flex flex-col items-center gap-2">
                        {vpn.logo_url && <img src={vpn.logo_url} alt={`${vpn.name} logo`} className="h-10 object-contain" />}
                      ...
                      <TableCell key={vpn.id} className="text-center text-base">
                        {(() => {
                          ...
                          if (typeof value === 'boolean') {
                            const displayValue = feature.key === 'keeps_logs' ? !value : value;
                            return <BooleanFeature value={displayValue} />;
                          }

                          if (feature.key === 'star_rating' && typeof value === 'number') {
                            return <div className="flex items-center justify-center">{value.toFixed(1)} <Star className="w-4 h-4 ml-1 text-yellow-500 fill-current" /></div>;
                          }
                          if (feature.key === 'price_monthly_usd' && typeof value === 'number') {
                  ...
                  {selectedVpns.map((vpn: Vpn) => (
                    <TableCell key={vpn.id} className="text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        {vpn.affiliate_link && (
                        <Button asChild size="lg" className="bg-green-500 text-white hover:bg-green-600 font-bold w-full">
                          <a 
          ...
          <div className="text-center py-8">
            <p className="text-muted-foreground">Select a VPN to see a comparison.</p>
          </div>
        )}
      </CardContent>
    </Card>
