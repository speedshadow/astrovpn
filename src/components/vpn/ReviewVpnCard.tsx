...
const FeatureItem = ({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) => (
...
const DeviceIcons = ({ devices }: { devices: Vpn['supported_devices'] }) => {
  ...
  const deviceMap: { [key: string]: React.ReactNode } = {
    windows: <span title="Windows"><Laptop size={18} /></span>,
    macos: <span title="macOS"><Apple size={18} /></span>,
    linux: <span title="Linux"><Bot size={18} /></span>,
    android: <span title="Android"><Smartphone size={18} /></span>,
    ios: <span title="iOS"><Smartphone size={18} /></span>,
  };
  ...
  return (
    <div className="flex items-center space-x-2">
      {supportedDevices.map((icon, i) => <div key={i}>{icon}</div>)}
    </div>
  );
};
    ...
    <div className="not-prose relative overflow-hidden rounded-xl border bg-card text-card-foreground shadow-lg transition-shadow hover:shadow-xl">
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          ...
          <div className="md:col-span-3 flex flex-col items-center text-center">
            ...
            <img src={vpn.logo_url ?? ''} alt={`${vpn.name} logo`} className="h-12 max-w-full object-contain mb-2" />
          ...
          <div className="md:col-span-3 flex flex-col items-center text-center space-y-4">
             {vpn.star_rating && (
              <div className="flex items-center space-x-1">
                <span className="text-2xl font-bold">{vpn.star_rating.toFixed(1)}</span>
                <div className="flex text-yellow-400">
                    {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`h-5 w-5 ${i < Math.round(vpn.star_rating!) ? 'fill-current' : 'text-gray-300'}`} />
                    ))}
                </div>
              </div>
            )}
      ...
      <div className="px-6 py-4 border-t bg-muted/30">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-5">
          ...
          {vpn.country_count && vpn.country_count > 0 && (
            <FeatureItem icon={<MapPin className="w-5 h-5" />} label="Countries">
              {vpn.country_count}
            </FeatureItem>
          )}
      ...
      {vpn.description && (
        <div className="px-6 py-4 bg-muted/50 border-t">
          <p className="text-sm text-muted-foreground italic">{vpn.description}</p>
        </div>
      )}
    </div>
  );
