...
import { ShieldCheck, MapPin, Server, ShieldAlert, Award, Gavel, CheckCircle, XCircle, Smartphone } from 'lucide-react';
...
export const VpnCard = ({ vpn, rank }: VpnCardProps) => {
  ...
  const shouldShowCountries = typeof vpn.country_count === 'number' && vpn.country_count > 0;
  const shouldShowSimultaneousConnections = typeof vpn.simultaneous_connections === 'number' && vpn.simultaneous_connections > 0;

  // Robustly parse prices to handle strings or numbers from the database
  const priceUSD = vpn.price_monthly_usd ? parseFloat(String(vpn.price_monthly_usd)) : NaN;
  const priceEUR = vpn.price_monthly_eur ? parseFloat(String(vpn.price_monthly_eur)) : NaN;
      ...
      >
        {rank !== undefined && rank < 3 && (
          <div className="absolute top-2 right-2 z-10">
            <Tooltip>
              <TooltipTrigger>
                <Award className={`h-6 w-6 ${rank === 0 ? 'text-amber-400 fill-amber-400/20' : rank === 1 ? 'text-slate-400 fill-slate-400/20' : 'text-orange-600 fill-orange-600/20'}`}/>
              </TooltipTrigger>
              <TooltipContent>
                <p>{rank === 0 ? '#1 Top Pick' : rank === 1 ? '#2 Runner-up' : '#3 Great Value'}</p>
              </TooltipContent>
            </Tooltip>
          </div>
        )}
        ...
        <div className="p-6 bg-card-foreground/5 flex items-center gap-4">
          {vpn.logo_url && <img src={vpn.logo_url} alt={`${vpn.name} logo`} className="h-12 w-12 object-contain" loading="lazy" width="48" height="48" />}
          <div>
            <h3 className="text-xl font-bold">{vpn.name}</h3>
            {typeof vpn.star_rating === 'number' && (
              <div className="mt-1"><StarRating rating={vpn.star_rating / 2} /></div>
            )}
            <div className="flex items-center gap-4 text-sm text-muted-foreground mt-2">
              {shouldShowServers && vpn.server_count && (
                <div className="flex items-center gap-1.5">
                  <Server className="size-4" />
                  <span><span className="font-semibold">{vpn.server_count.toLocaleString()}</span> Servers</span>
                </div>
              )}
              {shouldShowCountries && vpn.country_count && (
                <div className="flex items-center gap-1.5">
                  <MapPin className="size-4" />
                  <span><span className="font-semibold">{vpn.country_count}</span> Countries</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="p-6 flex-grow space-y-4">
          {vpn.categories && vpn.categories.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {vpn.categories.map(category => (
                <span key={category} className="px-2.5 py-1 text-xs font-semibold bg-primary/10 text-primary rounded-full">
                  {category}
                </span>
              ))}
            </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">{vpn.keeps_logs ? 'Keeps Logs' : 'No-Logs Policy'}</span>
            </div>
            {vpn.has_court_proof && (
              <div className="flex items-center gap-2">
            ...
            {vpn.based_in_country_name && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground">Based in {vpn.based_in_country_name}</span>
              </div>
            )}
          ...
          {shouldShowSimultaneousConnections && vpn.simultaneous_connections && (
            <div>
              <h4 className="font-semibold mb-2 text-sm">Simultaneous Connections:</h4>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Smartphone className="h-5 w-5" />
                <span>
                  {vpn.simultaneous_connections} device{vpn.simultaneous_connections === 1 ? '' : 's'} at once
                </span>
              </div>
            </div>
          )}
          ...
          <Button 
            onClick={async () => {
              ...
              if (vpn.affiliate_link) {
                window.open(vpn.affiliate_link, '_blank', 'noopener,noreferrer');
              }
            }}
