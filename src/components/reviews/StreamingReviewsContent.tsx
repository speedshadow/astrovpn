...
function StreamingReviewsContentComponent({ initialData }: StreamingReviewsContentProps) {
    ...
    <main className="container mx-auto px-4 py-12">
      ...
      <div className="lg:grid lg:grid-cols-[1fr_280px] lg:gap-12 items-start">
            ...
            <ul className="space-y-2 list-none p-0">
              {vpns.map((vpn, index) => (
                <li key={vpn.id} className="p-0 m-0">
                  <a href={`#vpn-${vpn.slug}`} className="flex items-center text-lg text-foreground hover:text-primary transition-colors">
            ...
            {vpns.map((vpn, index) => (
              <div key={vpn.id} id={`vpn-${vpn.slug}`} className="scroll-mt-24">
                <h2 className="text-3xl font-bold border-b pb-2 mb-6">
                  <span className="text-primary">#{index + 1}</span> - {vpn.name}
                </h2>
        ...
        <aside className="hidden lg:block sticky top-24">
          <div id="table-of-contents" className="p-6 bg-muted/50 rounded-lg border">
            <h3 className="text-xl font-bold mb-4">Table of Contents</h3>
            <ul className="space-y-2 list-none p-0">
              {vpns.map((vpn, index) => (
                <li key={vpn.id} className="p-0 m-0">
                  <a href={`#vpn-${vpn.slug}`} className="flex items-center text-foreground hover:text-primary transition-colors">
                    <span className="font-bold text-primary mr-3 w-6 text-center">{index + 1}</span>
