...
export const BlogCategoryManager = ({ initialCategories }: BlogCategoryManagerProps) => {
  ...
  const handleSave = async () => {
    ...
    if (categoryToEdit) {
      ...
      const { data, error } = await supabase
        ...
        .update({ name: newCategoryName, description: newCategoryDescription, slug })
      ...
      const { data, error } = await supabase
        .from('blog_categories')
        .insert([{ name: newCategoryName, description: newCategoryDescription, slug }])
        .select()
        .single();
          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="name" name="name" value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="description" className="text-right">Description</Label>
                <Textarea id="description" name="description" value={newCategoryDescription} onChange={(e) => setNewCategoryDescription(e.target.value)} className="col-span-3" />
              </div>
            </div>
            <DialogFooter>
              <DialogClose asChild>
            ...
            {categories.map(category => (
              <TableRow key={category.id}>
                ...
                <TableCell className="text-right">
                  <Button variant="outline" size="sm" className="mr-2" onClick={() => openDialogForEdit(category)}>
                    Edit
                  </Button>
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will permanently delete the category "{category.name}". Associated posts will not be deleted.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
