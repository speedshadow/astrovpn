...
const BackupManager: React.FC = () => {
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.details || result.error || 'Failed to create backup');
      }
  ...
  const handleRestore = async () => {
    ...
    if (!confirm(`Are you sure you want to restore from "${selectedFile.name}"? This action is irreversible.`)) return;
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.details || 'Failed to restore backup.');
  ...
  const handleDownload = async (filename: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/db/download-backup?filename=${filename}`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        }
      });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Use the tools below to create secure backups or restore your database from a <code>.dump</code> file.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Creates a secure backup of your database.</p>
            <Button onClick={handleCreateBackup} disabled={isCreating || isRestoring || !!isDownloading}>
              {isCreating ? 'Creating...' : 'Create Secure Backup'}
            </Button>
          </div>
          <div className="space-y-2">
            <h4 className="font-medium">Restore from Backup</h4>
            <p className="text-sm text-muted-foreground">Restore from a <code>.dump</code> file. This overwrites all data.</p>
            <div className="flex items-center space-x-2">
              ...
              />
              <Button onClick={handleRestore} disabled={isRestoring || isCreating || !selectedFile || !!isDownloading}>
                  ...
                  backups.map((backup) => (
                    <TableRow key={backup}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="outline" size="sm" onClick={() => handleDownload(backup)} disabled={!!isDownloading}>
                          {isDownloading === backup ? 'Downloading...' : 'Download'}
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow><TableCell colSpan={2} className="text-center">No backups found.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
