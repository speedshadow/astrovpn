...
export function DatabaseManager() {
  ...
  const fetchBackups = async () => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to fetch backups.');
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      toast.success('Backup created successfully!', { description: result.file });
  ...
  const handleRestore = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success('Database restored successfully!', { description: `The page will now reload to reflect the changes.` });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage manual backups for your local database. These files are stored in the `supabase/backups` directory.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-4 mb-4">
            ...
            <Button asChild variant="outline">
              <Label htmlFor="restore-upload">
                ...
                <Input id="restore-upload" type="file" className="hidden" onChange={handleRestore} accept=".sql" disabled={isRestoring} />
              </Label>
            </Button>
          </div>
          <Card>
            <Table>
              ...
              <TableBody>
                {isFetching ? (
                  <TableRow><TableCell colSpan={4} className="text-center">Loading backups...</TableCell></TableRow>
                ) : backups.length > 0 ? (
                  backups.map((backup) => (
                    <TableRow key={backup.name}>
                      ...
                      <TableCell className="text-right">
                        <Button asChild variant="ghost" size="icon">
                          <a href={`/api/db/download?file=${backup.name}`} download>
                        ...
                        <Button
                          ...
                          className="h-8 w-8"
                        >
                          {deletingFile === backup.name ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4 text-destructive" />}
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure automated backups for the production environment. This only saves the configuration; a separate cron job on the server is required to execute the backups.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value: string) => setBackupConfig({ ...backupConfig, schedule: value })}
            >
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setBackupConfig({ ...backupConfig, time: e.target.value })}
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setBackupConfig({ ...backupConfig, storagePath: e.target.value })}
