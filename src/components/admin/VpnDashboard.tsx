              ...
              vpns.map((vpn) => (
                <TableRow key={vpn.id}>
                  <TableCell className="font-medium flex items-center gap-3">
                    {vpn.logo_url ? (
                      <img src={vpn.logo_url} alt={`${vpn.name} logo`} className="h-8 w-8 object-contain rounded-full" />
                    ) : (
                  ...
                  <TableCell>{vpn.based_in_country_flag} {vpn.based_in_country_name ?? 'N/A'}</TableCell>
                  <TableCell>{vpn.show_on_homepage ? 'Yes' : 'No'}</TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1 max-w-xs">
                      {vpn.categories && vpn.categories.length > 0 ? (
                        vpn.categories.map(category => (
                          <span key={category} className="px-2 py-0.5 text-xs font-medium bg-secondary text-secondary-foreground rounded-full">
                            {category}
                          </span>
                        ))
                      ) : (
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleDelete(vpn.id)} className="text-destructive focus:text-destructive">
                          Delete
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
