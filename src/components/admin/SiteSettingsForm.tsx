...
const settingsSchema = z.object({
  ...
  social_preview_image_url: z.string().url('Must be a valid URL').optional().or(z.literal('')),
...
export const SiteSettingsForm = ({ initialData }: SiteSettingsFormProps) => {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<SettingsFormValues>({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Configure your site's identity and SEO settings.</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          >
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                />
                {errors.site_title && (
                  <p className="text-sm text-red-500">{errors.site_title.message}</p>
                )}
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                />
                {errors.site_url && (
                  <p className="text-sm text-red-500">{errors.site_url.message}</p>
                )}
              </div>
              <div className="space-y-2">
                <Label htmlFor="meta_description">Meta Description</Label>
                <Textarea id="meta_description" {...register('meta_description')} placeholder="A brief summary of your site for search engines." />
              </div>
              <div className="space-y-2">
                <Label htmlFor="meta_keywords">Meta Keywords</Label>
                <Input id="meta_keywords" {...register('meta_keywords')} placeholder="vpn, security, privacy, reviews" />
              ...
              <div className="space-y-2">
                ...
                />
                {errors.favicon_url && (
                  <p className="text-sm text-red-500">{errors.favicon_url.message}</p>
                )}
                <p className="text-sm text-muted-foreground">Recommended: SVG or PNG, 32x32 pixels</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="social_preview_image_url">Social Preview Image URL</Label>
                <Input 
                  id="social_preview_image_url" 
                  type="url" 
                  {...register('social_preview_image_url')} 
                  placeholder="https://example.com/social-image.png"
                  className={errors.social_preview_image_url ? 'border-red-500' : ''}
                />
                {errors.social_preview_image_url && (
                  <p className="text-sm text-red-500">{errors.social_preview_image_url.message}</p>
                )}
                <p className="text-sm text-muted-foreground">Recommended: 1200x630 pixels for optimal social media display</p>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  defaultValue=""
                />
                <p className="text-sm text-muted-foreground">Ex: G-XXXXXXXXXX or UA-XXXXXXXX-X</p>
              </div>
              <Button 
                type="submit" 
