...
export const ReviewForm = ({ allVpns, reviewPageToEdit }: ReviewFormProps) => {
  ...
  const [formData, setFormData] = useState<Omit<ReviewPage, 'id' | 'created_at'>>({
  ...
  const [selectedVpns, setSelectedVpns] = useState<SelectableVpn[]>(initialSelected);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reviewPageToEdit) {
      ...
      // Create new page
      const { error: insertError } = await supabase.from('review_pages').insert(submissionData);
      error = insertError;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="bg-card p-6 rounded-lg shadow-md border space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">URL-friendly identifier. E.g., /reviews/{formData.slug}</p>
        </div>
        <div>
          <label htmlFor="description" className="block text-sm font-medium mb-1">Description (for SEO)</label>
        ...
        <div>
          ...
          <Textarea id="introduction" name="introduction" value={formData.introduction ?? ''} onChange={handleInputChange} rows={5} />
        </div>
        <div>
          <label htmlFor="conclusion" className="block text-sm font-medium mb-1">Conclusion</label>
          <Textarea id="conclusion" name="conclusion" value={formData.conclusion ?? ''} onChange={handleInputChange} rows={5} />
        </div>
      </div>
      ...
      <div className="bg-card p-6 rounded-lg shadow-md border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="border rounded-md h-96 overflow-y-auto">
              {availableVpns.map(vpn => (
                <button type="button" key={vpn.id} onClick={() => handleSelectVpn(vpn)} className="flex items-center w-full text-left p-2 border-b cursor-pointer hover:bg-muted">
                  <img src={vpn.logo_url ?? ''} alt={`${vpn.name} logo`} className="w-8 h-8 mr-3 object-contain" />
                  <span>{vpn.name}</span>
                </button>
              ))}
            </div>
          ...
          {/* Selected VPNs */}
          <div>
            <h3 className="font-semibold mb-2">Selected VPNs (Drag to Reorder)</h3>
            <div className="border rounded-md h-96 overflow-y-auto">
              <Reorder.Group axis="y" values={selectedVpns} onReorder={setSelectedVpns}>
                {selectedVpns.map((vpn) => (
                  <Reorder.Item key={vpn.id} value={vpn} className="p-2 border-b bg-card">
                    <div className="flex items-center justify-between">
                        ...
                        <Button type="button" variant="ghost" size="sm" onClick={() => handleDeselectVpn(vpn)}>Remove</Button>
                    </div>
                  </Reorder.Item>
                ))}
              </Reorder.Group>
      ...
      <div className="flex justify-end">
        <Button type="submit" disabled={isLoading}>
          {isLoading ? 'Saving...' : (reviewPageToEdit ? 'Update Page' : 'Create Page')}
        </Button>
      </div>
    </form>
  );
