...
const getLinkClass = (path: string, pathname: string) => {
  const baseClass = 'mx-[-0.65rem] flex items-center gap-4 rounded-xl px-3 py-2 text-muted-foreground hover:text-foreground';
    ...
    <Sheet>
      ...
      <SheetContent side="left" className="flex flex-col">
        <nav className="grid gap-2 text-lg font-medium">
          ...
          {navLinks.map(link => (
            <a key={link.href} href={link.href} className={getLinkClass(link.href, pathname)}>
