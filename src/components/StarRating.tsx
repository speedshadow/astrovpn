...
export const StarRating = ({ rating: rawRating, size = 20 }: StarRatingProps) => {
  ...
  return (
    <div className="flex items-center gap-1 text-yellow-400">
      ...
      {Array.from({ length: emptyStars }).map((_, i) => (
        <Star key={`empty-${i}`} className="text-gray-300 dark:text-gray-600" style={{ width: size, height: size }} />
      ))}
    </div>
  );
};
