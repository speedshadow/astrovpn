...
export const CommentSection = ({ postId }: CommentSectionProps) => {
  ...
  const handleCommentSubmit = async () => {
    ...
    if (session) {
      // User is logged in
      commentData = { post_id: postId, content: newComment, author_id: session.user.id };
    } else {
      // Guest is commenting
      if (!guestName.trim()) {
      ...
      commentData = { post_id: postId, content: newComment, guest_name: guestName };
    }
    ...
    if (error) {
      ...
      if (session && error.code === '23503' && error.message.includes('blog_comments_author_id_fkey')) {
        alert('Your login session is out of date. Please log in again to comment.');
    ...
    <div className="mt-12">
      ...
      <div className="mb-8">
        ...
        <Textarea
          ...
          placeholder={session ? `Commenting as ${session.user.user_metadata.full_name}...` : 'Write a comment as a guest...'}
          className="mb-2"
        />
          ...
          <div key={comment.id} className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center">
                <p className="font-semibold">{comment.author?.full_name || comment.guest_name || 'Anonymous'}</p>
